...
class Controlled_World(World):
    ...
    def __init__(self, **kwargs):
        super(Controlled_World, self).__init__(**kwargs)
    ...
    def step(self):
        super(Controlled_World, self).step()
        self.resetActor()
...
if __name__ == "__main__":
    ...
    if fwSettings.backend=='pyglet':
        from gym_hNs.backends.pyglet_framework import PygletFramework as Framework
    elif fwSettings.backend=='no_render':
        from gym_hNs.backends.no_render import NoRender as Framework
    framework = Framework(fwSettings)
    import time
    import gym_hNs.envs.bots as bots
    n_agents = [10, 10, 10]
    actors = [[bots.bot0] * n_agents[1]] * len(n_agents)

    s = time.time()
    if fwSettings.backend == 'no_render':
        world = World(framework = framework, n_agents = n_agents, actors=actors)

        while not world.done:
            # actions = world.action_space.sample()
            world.step()
    else:
        world = Controlled_World(framework = framework, n_agents = n_agents, actors=actors)
        world.framework.run()
